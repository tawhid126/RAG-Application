...
async def lifespan(app: FastAPI):
    ...
    logger.info("=" * 60)
    # Initialize Postgres chat history table
    if settings.postgres_url:
        try:
            from app.services.postgres_history import init_db
            init_db()
            logger.info("Neon Postgres chat history: ready")
        except Exception as e:
            logger.warning(f"Postgres init failed (history disabled): {e}")
    yield
    # Shutdown
